...
const HeroSection = ({ name, onCreatePost, onExplore }: { name: string, onCreatePost?: () => void, onExplore?: () => void }) => {
    ...
    return (
        <div className="relative w-full rounded-2xl sm:rounded-3xl lg:rounded-[3rem] p-6 sm:p-8 md:p-10 lg:p-14 xl:p-16 mb-8 sm:mb-12 overflow-hidden shadow-2xl group ring-1 ring-white/20 dark:ring-white/5 bg-gradient-to-br from-gray-900 to-black">
            {/* Dynamic Animated Background Elements */}
            <div className="absolute inset-0 overflow-hidden">
                <div className="absolute top-[-20%] left-[-10%] w-[300px] sm:w-[400px] lg:w-[500px] h-[300px] sm:h-[400px] lg:h-[500px] bg-brand-orange/20 rounded-full blur-[60px] sm:blur-[80px] lg:blur-[100px] animate-pulse-slow"></div>
                <div className="absolute bottom-[-20%] right-[-10%] w-[300px] sm:w-[400px] lg:w-[500px] h-[300px] sm:h-[400px] lg:h-[500px] bg-blue-600/20 rounded-full blur-[60px] sm:blur-[80px] lg:blur-[100px] animate-pulse-slow" style={{ animationDelay: '2s' }}></div>
                <div className="absolute top-[20%] right-[20%] w-[200px] sm:w-[250px] lg:w-[300px] h-[200px] sm:h-[250px] lg:h-[300px] bg-purple-500/10 rounded-full blur-[50px] sm:blur-[60px] lg:blur-[80px] animate-float-slow"></div>
            </div>

            {/* Floating 3D Emojis with Motion - Hidden on mobile and small tablets */}
            <div className="absolute top-6 sm:top-10 right-6 sm:right-10 lg:right-20 animate-float-slow hidden lg:block z-10">
                <div className="text-6xl lg:text-8xl drop-shadow-2xl opacity-90 hover:scale-110 transition-transform cursor-pointer rotate-12 hover:rotate-[20deg] duration-500">🍕</div>
            </div>
            <div className="absolute bottom-12 sm:bottom-16 right-1/4 sm:right-1/3 animate-float-medium hidden lg:block z-10">
                <div className="text-5xl lg:text-7xl drop-shadow-2xl opacity-80 hover:scale-110 transition-transform cursor-pointer -rotate-12 hover:-rotate-[20deg] duration-500">🥑</div>
            </div>
            <div className="absolute top-1/2 right-6 sm:right-10 animate-float-fast hidden lg:block z-10">
                <div className="text-4xl lg:text-6xl drop-shadow-2xl opacity-70 hover:scale-110 transition-transform cursor-pointer rotate-[25deg] hover:rotate-[45deg] duration-500">🥘</div>
            </div>
            <div className="absolute bottom-6 sm:bottom-8 left-1/4 sm:left-1/3 animate-float-slow hidden lg:block z-10" style={{ animationDelay: '1s' }}>
                <div className="text-4xl lg:text-5xl drop-shadow-2xl opacity-60 hover:scale-110 transition-transform cursor-pointer -rotate-[15deg] duration-500">🥐</div>
            </div>

            <div className="relative z-20 max-w-full sm:max-w-2xl lg:max-w-3xl">
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6 animate-fade-up" style={{ animationDelay: '0.1s' }}>
                    <div className="bg-white/10 backdrop-blur-md border border-white/20 px-2 sm:px-3 py-1 rounded-full flex items-center gap-1.5 sm:gap-2">
                        <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-brand-green rounded-full animate-pulse"></span>
                        <span className="text-white text-[9px] sm:text-[10px] font-black uppercase tracking-widest">
                            Live Kitchen
                        </span>
                    </div>
                </div>

                <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black text-white leading-[0.9] sm:leading-[1] mb-4 sm:mb-6 animate-fade-up tracking-tighter" style={{ animationDelay: '0.2s' }}>
                    {greeting}, <br />
                ...
                <p className="text-gray-400 text-sm sm:text-base md:text-lg lg:text-xl mb-6 sm:mb-8 lg:mb-10 max-w-full sm:max-w-lg leading-relaxed animate-fade-up font-medium" style={{ animationDelay: '0.3s' }}>
                    Unleash your culinary creativity. Share flavors, discover recipes, and connect with foodies worldwide.
                </p>

                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 animate-fade-up" style={{ animationDelay: '0.4s' }}>
                    <button
                        onClick={onCreatePost}
                        className="bg-brand-orange text-white px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-bold shadow-lg shadow-brand-orange/20 hover:bg-white hover:text-brand-orange hover:-translate-y-1 transition-all duration-300 flex items-center justify-center gap-2 group/btn relative overflow-hidden min-h-[48px]"
                    >
                        <span className="relative z-10 flex items-center gap-2 text-sm sm:text-base">Create Recipe <ArrowRight size={16} className="sm:w-[18px] sm:h-[18px] group-hover/btn:translate-x-1 transition-transform" /></span>
                        <div className="absolute inset-0 bg-white opacity-0 group-hover/btn:opacity-10 transition-opacity"></div>
                    </button>
                    <button
                        onClick={onExplore}
                        className="px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-bold text-white border border-white/10 hover:bg-white/10 transition-all duration-300 backdrop-blur-sm flex items-center justify-center gap-2 text-sm sm:text-base min-h-[48px]"
                    >
                        Explore Community
                    </button>
                </div>
...
const StoryRail = ({ stories, currentUser, onOpenLive, onGoLive, onViewStory, onDeleteStory }: { stories: Story[], currentUser?: User, onOpenLive: (u: User) => void, onGoLive: () => void, onViewStory: (idx: number) => void, onDeleteStory?: (id: string) => void }) => {
    return (
        <div className="relative mb-8 sm:mb-12">
            <div className="flex gap-3 sm:gap-4 lg:gap-6 overflow-x-auto pb-4 sm:pb-6 pt-2 px-2 no-scrollbar mask-linear items-start">
                {/* Go Live / Create Story Button */}
                <div className="flex flex-col items-center gap-2 sm:gap-3 min-w-[70px] sm:min-w-[80px] cursor-pointer group" onClick={onGoLive}>
                    <div className="w-[70px] h-[70px] sm:w-[80px] sm:h-[80px] rounded-2xl sm:rounded-[2.5rem] border-2 border-dashed border-gray-300 dark:border-gray-700 flex items-center justify-center bg-white dark:bg-gray-800 relative transition-all duration-300 group-hover:-translate-y-2 group-hover:border-brand-orange group-hover:shadow-xl group-hover:shadow-brand-orange/10">
                        <div className="absolute inset-0 bg-brand-orange/5 rounded-2xl sm:rounded-[2.5rem] group-hover:bg-brand-orange/10 transition"></div>
                        <div className="bg-brand-orange text-white p-2 sm:p-2.5 rounded-full shadow-lg group-hover:scale-110 transition-transform">
                            <Video size={18} className="sm:w-[22px] sm:h-[22px]" />
                        </div>
                        <div className="absolute -bottom-1 -right-1 bg-brand-green text-white p-0.5 sm:p-1 rounded-full border-2 border-white dark:border-gray-800">
                            <Zap size={8} className="sm:w-[10px] sm:h-[10px]" fill="currentColor" />
                        </div>
                    </div>
                    <span className="text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white transition-colors">Go Live</span>
                </div>
                ...
                {stories.map((story, idx) => {
                    ...
                    return (
                        <div
                            key={story.id}
                            className="flex flex-col items-center gap-2 sm:gap-3 min-w-[70px] sm:min-w-[80px] cursor-pointer group animate-fade-up relative"
                            style={{ animationDelay: `${idx * 0.1}s` }}
                            onClick={() => story.isLive ? onOpenLive(story.user) : onViewStory(idx)}
                        >
                            <div className={`w-[70px] h-[70px] sm:w-[80px] sm:h-[80px] p-[2px] sm:p-[3px] rounded-2xl sm:rounded-[2.5rem] relative transition-all duration-300 group-hover:-translate-y-2 group-hover:shadow-xl ${story.isLive ? 'bg-gradient-to-tr from-red-500 via-orange-500 to-yellow-500 animate-pulse' : story.isViewed ? 'bg-gray-200 dark:bg-gray-700' : 'bg-gradient-to-tr from-brand-orange via-pink-500 to-purple-500'}`}>
                                <div className="w-full h-full rounded-[1.8rem] sm:rounded-[2.2rem] overflow-hidden border-[2px] sm:border-[3px] border-white dark:border-gray-800 bg-white dark:bg-gray-800 relative">
                                    <img
                                ...
                                {story.isLive && (
                                    <div className="absolute -bottom-1.5 sm:-bottom-2 left-1/2 -translate-x-1/2 bg-red-600 text-white text-[8px] sm:text-[9px] px-1.5 sm:px-2 py-0.5 rounded-full font-black border-2 border-white dark:border-gray-800 shadow-sm z-10 tracking-wider">
                                        LIVE
                                    </div>
                                )}
                            </div>
                            ...
                            {isOwner && onDeleteStory && !story.isLive && (
                                <button
                                    ...
                                    className="absolute top-0 right-0 bg-white dark:bg-gray-800 text-red-500 rounded-full p-1 sm:p-1.5 shadow-md border border-gray-100 dark:border-gray-700 z-20 hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors opacity-0 group-hover:opacity-100 scale-75 group-hover:scale-100 duration-200 min-h-[32px] min-w-[32px] flex items-center justify-center"
                                    title="Delete Story"
                                >
                                    <X size={8} className="sm:w-[10px] sm:h-[10px]" strokeWidth={3} />
                                </button>
                            )}

                            <span className="text-[10px] sm:text-xs font-bold text-gray-600 dark:text-gray-400 truncate w-16 sm:w-20 text-center group-hover:text-brand-orange transition-colors">
...
const Feed: React.FC<FeedProps> = ({ stories, posts, currentUser, onOpenRecipe, onOpenLive, onGoLive, onViewStory, onToggleLike, onViewAll, onDeletePost, onDeleteStory, onEditPost, onCreatePost, onExplore }) => {
    ...
    return (
        <div className="flex flex-col w-full max-w-none sm:max-w-2xl md:max-w-4xl lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl mx-auto">
            <TrendingTicker />
            ...
            <div className="px-0 sm:px-4 lg:px-8">
                ...
                <div className="mb-8 sm:mb-12">
                    <h2 className="text-lg sm:text-xl font-black text-gray-900 dark:text-white mb-4 sm:mb-6 flex items-center gap-2 sm:gap-3 px-4 sm:px-0">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-tr from-brand-orange to-pink-500 rounded-lg sm:rounded-xl flex items-center justify-center text-white shadow-lg shadow-orange-200 dark:shadow-none">
                            <CircleDashed size={16} className="sm:w-5 sm:h-5" />
                        </div>
                        Kitchen Stories
                    </h2>
                ...
                <section className="mb-12 sm:mb-16 px-4 sm:px-0">
                    <div className="flex items-center justify-between mb-6 sm:mb-8">
                        <h2 className="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white flex items-center gap-2 sm:gap-3 tracking-tight">
                            <TrendingUp className="text-brand-orange" size={24} className="sm:w-8 sm:h-8" />
                            Trending Now
                        </h2>
                        <button onClick={onViewAll} className="group flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm font-bold text-gray-500 dark:text-gray-400 hover:text-brand-orange transition-colors bg-white dark:bg-gray-800 px-3 sm:px-4 py-2 rounded-full border border-gray-100 dark:border-gray-700 shadow-sm min-h-[40px]">
                            View All <ArrowRight size={12} className="sm:w-[14px] sm:h-[14px] group-hover:translate-x-1 transition-transform" />
                        </button>
                    </div>
                ...
                <section className="px-4 sm:px-0">
                    <div className="flex items-center gap-2 sm:gap-3 mb-6 sm:mb-8">
                        <div className="p-2 sm:p-3 bg-blue-500/10 rounded-xl sm:rounded-2xl">
                            <Sparkles className="text-blue-500 fill-blue-500" size={20} className="sm:w-6 sm:h-6" />
                        </div>
                        <h2 className="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white tracking-tight">Fresh from the Kitchen</h2>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
